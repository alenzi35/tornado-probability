...
# ---------------- LCC PROJECTION ----------------
lcc_proj = Proj(proj='lcc', lat_1=25, lat_2=60, lat_0=38, lon_0=-97, x_0=0, y_0=0, units='m')

# ---------------- COMPUTE PROBABILITY ----------------
linear = INTERCEPT + COEFFS["CAPE"]*cape + COEFFS["CIN"]*cin + COEFFS["HLCY"]*hlcy
prob = 1 / (1 + np.exp(-linear))

# ---------------- WRITE JSON WITH METERS ----------------
features = []
...
for i in range(rows):
    for j in range(cols):
        lat = float(lats[i,j])
        lon = float(lons[i,j])
        x, y = lcc_proj(lon, lat)  # convert to meters
        features.append({
            "x": x,
            "y": y,
            "prob": float(prob[i,j])
...
print("✅ Tornado probability JSON (LCC meters) written to:", OUTPUT_JSON)
